...
impl Guess {
    // Called on a Guess struct instance
    ...
    pub fn play_game_round(&mut self) -> RoundResult {
        ...
        if guess && self.current_number < next_number || !guess && self.current_number > next_number {
            ...
            // Update Guess struct (game state)
...
// Creates and returns a new Guess struct instance (technically a move???)
pub fn start_game() -> Guess {
    Guess {
        rounds: 0,
